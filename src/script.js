// Animal Class
class Animal {
  constructor(name, image, numberOfLegs) {
    this.name = name;
    this.image = image;
    this.numberOfLegs = numberOfLegs;
  }
}

// Dog Class
class Dog extends Animal {
  constructor(name, image, numberOfLegs, age) {
    super(name, image, numberOfLegs);
    this.age = age;
  }
}

// Duck Class
class Duck extends Animal {
  constructor(name, image, numberOfLegs, age) {
    super(name, image, numberOfLegs);
    this.age = age;
  }
}

// Dogs
const dog1 = new Dog(
  "Driver",
  "https://www.petihtiyac.com/Data/Blog/19.jpg",
  "4",
  "8"
);
const dog2 = new Dog(
  "Bobo",
  "data:image/jpeg;base64,",
  "4",
  "4"
);
const dog3 = new Dog(
  "Gandalf",
  "data:image/jpeg;base64,",
  "4",
  "2"
);

// Ducks
const duck1 = new Duck(
  "White",
  "https://mediacat.com/wp-content/uploads/2019/01/disneyland-donald-duck-reklami-00.jpg",
  "2",
  "5"
);
const duck2 = new Duck(
  "Moninini",
  "https://bilgiherseydir.com/files/uploads/news/default/duck-syndrom-ordek-s-8222d64809529d077071.jpg",
  "2",
  "3"
);
const duck3 = new Duck(
  "Rikisa",
  "https://i.pinimg.com/originals/4e/90/e9/4e90e961105b3112230b32c3946843e7.jpg",
  "2",
  "2"
);

const dogs = [];
const ducks = [];

// Push Animals To Arrays
dogs.push(dog1, dog2, dog3);
ducks.push(duck1, duck2, duck3);

const dogCards = document.querySelector("#dog__cards");
const duckCards = document.querySelector("#duck__cards");

// Render Dogs
dogCards.innerHTML = dogs.map(
  (dog) =>
    `<div class="card">
      <img src=${dog.image}></img>
      <p>${dog.constructor.name}</p>
      <p><span class="card__text">Name : </span> ${dog.name}</p>
      <p><span class="card__text">Number Of Legs : </span> ${dog.numberOfLegs}</p>
      <p><span class="card__text">Age : </span>${dog.age}</p>
    </div>`
);

// Render Ducks
duckCards.innerHTML = ducks.map(
  (duck) =>
    `<div class="card">
      <img src=${duck.image}></img>
      <p>${duck.constructor.name}</p>
      <p><span class="card__text">Name : </span>${duck.name}</p>
      <p><span class="card__text">Number Of Legs : </span> ${duck.numberOfLegs}</p>
      <p><span class="card__text">Age : </span> ${duck.age}</p>
    </div>`
);